...
<head>
  ...
  <title>${curProject.name }::编辑测试任务 - BUG管理</title>
<script language='Javascript'>var config={"webRoot":"\/","cookieLife":30,"requestType":"PATH_INFO","pathType":"clean","requestFix":"-","moduleVar":"m","methodVar":"f","viewVar":"t","defaultView":"html","themeRoot":"\/theme\/","currentModule":"testtask","currentMethod":"edit","clientLang":"zh-cn","requiredFields":"project,build,begin,end,name","submitting":"\u7a0d\u5019...","save":"\u4fdd\u5b58","router":"\/index.php\/testtask-edit-37.html"}
</script>
...
<script language='javascript'>
var editor = {"id":["desc","report"],"tools":"simpleTools"};
...
<form method='post' action="updateTask.htm" id='dataform'>
  ...
  <table class='table-1'> 
    ...
    <tr>
      ...
      <select name='versionId' id='build' class=select-3>
		...
		<c:forEach items="${versions }" var="version">
			<c:choose>
			<c:when test="${task.version.versionId eq version.versionId }">
				<option value="${version.versionId }"  selected='selected'>${version.name }</option>
			</c:when>
			<c:otherwise>
    ...
    <tr>
      ...
      <td><select name='ownerId' id='owner'>
		...
		<c:forEach items="${users }" var="user">
			<c:choose>
			<c:when test="${task.owner.testerId eq user.userId }">
    ...
    <tr>
      ...
      <td><input type='text' name='startDate' id='begin' value='<fmt:formatDate pattern="yyyy-MM-dd" value="${task.startDate }" />' class='text-3 date' />
</td>
    </tr>  
    <tr>
      <th class='rowhead'>结束日期</th>
      <td><input type='text' name='endDate' id='end' value='<fmt:formatDate pattern="yyyy-MM-dd" value="${task.endDate }" />' class='text-3 date' />
</td>
    </tr>  
    <tr>
    ...
    <tr>
      ...
      <td><input type='text' name='name' id='name' value='${task.name }' class='text-1' />
</td>
    </tr>  
    <tr>
      <th class='rowhead'>任务描述</th>
      <td><textarea name='description' id='desc' rows=10 class='area-1'>${task.description }</textarea>
</td>
    </tr>
     <tr>
      <th class='rowhead'>测试总结</th>
      <td><textarea name='summary' id='report' rows=10 class='area-1'>${task.summary }</textarea>
</td>
    </tr>
    <tr>
      <td colspan='2' class='a-center'> <input type='submit' id='submit' value='更新'  class='button-s' /> <input type='button' onClick='javascript:history.go(-1);' value='返回' class='button-b' /> </td>
    </tr>
  </table>
</form>
  </div>
