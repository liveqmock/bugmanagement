...
<script> 
noResultsMatch    = '没有匹配结果';
chooseUsersToMail = '选择要发信通知的用户...';
...
<script language='Javascript'>holders = {"chooseBuilds":"\u9009\u62e9\u76f8\u5173\u7248\u672c..."}
</script>
<script language='Javascript'>page = "assignedto"
</script>
<form method='post' id="bugsheet" target="theID" >
  <table class='table-1'>
    <caption>${bug.title}</caption>
    <tr>
      <th class='rowhead'>指派给</th>
      <td>
          <select name='assignedTo' id='assignedTo' class='text-3'>
          <option value=''></option>
          <c:forEach var="x" items="${developers}" varStatus="status">
          <option value='${x.developerId}'>${status.index+1}. ${x.user.realName}</option>
          </c:forEach>
    ...
    <tr>
      <td class='rowhead'>抄送给</td>
      <td>
        <select name='mailto[]' id='mailto' class="w-p98" multiple>
          <option value='' selected='selected'></option>
          <c:forEach var="x" items="${developers}" varStatus="status">
          <option value='${x.user.userId}'>${status.index+1}. ${x.user.realName}</option>
          </c:forEach>
          <option value='closed'>Closed</option>
          </select>
       </td>
