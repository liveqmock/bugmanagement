...
<body>
	...
	<div class="navbar" id="modulemenu">
		<ul>
			...
			<li class=" "><a href="" target="" id="submenubrowse">浏览Bug</a></li>
				...
				<table class="table-1">
					...
					<tbody>
						<tr>
							...
							<td><select name="product" id="product"class="select-3"
								autocomplete="off">
								<option value="${bug.module.project.projectId}" selected="selected">${bug.module.project.name}</option>
						...
						<tr>
							...
							<td><span id="buildBox"> 
							<select name="version" id="version" size="5" multiple="multiple" class="select-3">
									<c:forEach items="${versionList}" var="versionList">
									<c:choose>
										<c:when test="${aList.contains(versionList)}">
									
										<option value="${versionList.versionId}" selected="selected">${versionList.name}</option>
										
										</c:when>
										<c:otherwise>
										
										<option value="${versionList.versionId}">${versionList.name}</option>
										
										</c:otherwise> 
									</c:choose>
									</c:forEach>	
							</select>
								<span class="star"> * </span> <a href="" target="_blank">创建发布
								</a> <a href="javascript:loadProductBuilds(36)">刷新</a></span></td>
						</tr>
						<tr>
							<th class="rowhead"><nobr>当前指派</nobr></th>
							<td><span id="assignedToBox">
							<select name="assignedTo" id="assignedTo" class="select-3">
						...
						<tr>
							...
							<td><textarea class="xheditors" id="activeContentRichEditor"
									name="chongxian" style="height: 200px; width: 61%;">${bug.steps}</textarea>
							</td>
						</tr>
								...
								<div id="mailto_chzn"
									...
									<ul class="chzn-choices">
											<li class="search-field"><input type="text"
												...
												style="width: 176px;display: none;">
											</li> 
										 <c:forEach items="${userlist}" var="userlist">
									...
									<div class="chzn-drop"
										style="width: 100%; top: 29px;display: none;">
						...
						<tr>
							...
							<td><input type="text" name="keywords" id="keywords"
								value="${bug.keyword}" class="text-1"></td>
						</tr>
						<tr>
	...
	<div id="footer">
		<div id="crumbs">
			<a href="turnToHomePage.htm">Bug管理</a> &nbsp;<span
