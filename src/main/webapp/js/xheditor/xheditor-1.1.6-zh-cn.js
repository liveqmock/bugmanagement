...
var toolsThemes={
	...
	mymini:'Blocktag,Fontface,FontSize,Bold,Italic,Underline,Strikethrough,FontColor,BackColor,|,Align,List,Outdent,Indent,|,Img,Emot',
...
$.xheditor=function(textarea,options)
{
	...
	if(settings.tools.match(/^\s*(m?full|simple|mini|mymini)\s*$/i))
	{
	...
	if(!settings.tools.match(/(^|,)\s*About\s*(,|$)/i))settings.tools+='';
	...
	{
		if( url!=undefined&&url.match(/^(\w+):\/\//i) && !url.match(/^https?:/i) )return url;
...
$(function(){
	...
	$('textarea').each(function(){
		var self=$(this),xhClass=self.attr('class').match(/(?:^|\s)xheditor(?:\-(m?full|simple|mini|mymini))?(?:\s|$)/i);
